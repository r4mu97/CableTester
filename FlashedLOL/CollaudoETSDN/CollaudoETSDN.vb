...
Public Class MainEtsdn
    ...
    Private Delegate Sub ChangeControlRichTextDelegate(ByVal ctrl As RichTextBox, ByVal text As String, val As Integer)
    Public Sub ChangeControlRichText(ByVal ctrl As RichTextBox, ByVal text As String, val As Integer)
        If Me.InvokeRequired Then
            Try
    ...
    Function SelectTests()
        ...
        Catch ex As Exception
            ChangeControlRichText(RichTextBox1, ex.ToString, 1)
        End Try
    End Function
    ...
    Sub MyBackgroundThreadETSDN()
        ...
        ChangeControlRichText(RichTextBox1, "Test Start " & Date.Now.ToLongTimeString, 0)
        ...
        ChangeControlRichText(RichTextBox1, "Test Finished " & Date.Now.ToLongTimeString, 0)
        'cambio colore alla barra in base al risultato di tutti i tets 
        If fail Then
            ChangeBarColor(BarTestEtsdn, ProgressBarColor.Red)
        Else
            ChangeBarColor(BarTestEtsdn, ProgressBarColor.Green)
        ...
        '    If enable Then
        '        AddRow(DataGridView1, VarRxFromETSDN.GetIntance.etsdnSN, Date.Now.ToLongTimeString)
        '        enable = False
        '    End If

        '    If result.IsSuccesfull Then
        '        AddRow(DataGridView1, result.NameTest, "Pass")
        '    Else
        '        AddRow(DataGridView1, result.NameTest, "NotPass", result.ErrorType)
        '    End If

        'Next

        'writeCSV.WriteCSVfile(Me)


        'commitDeviceCraft()
        ', result.IsSuccesfull.ToString, result.ErrorType.ToString

        'If result.ErrorType <> "" Then
        '    AddRow(DataGridView1, result.ErrorType)
        'Else
        ...
        'Dim val = My.Computer.FileSystem.OpenTextFileWriter(path, True)
        'For Each result In results
        '    val.WriteLine(result.NameTest().ToString & ";" & result.IsSuccesfull())
        'Next
        'val.Close()



        'While Not stato.RequestToCloseTest

        '    If pBoxStart.Visible = False Then
        'executor.Execute_test_suite(suite, Me)
        '        ChangeControlVisibility(pBoxStart, True)
        '    End If

        'End While
        'ChangeControlVisibility(pBoxStart, True)
        'ChangeControlVisibility(pBoxLoading, False)




        'aggiorno la grafica dei valore nelle label

        'While stato.RequestToCloseTest = False
        '    Dim results As List(Of TestResult)
        '    If  Then
        ...
        'check comunicazione can con il dispositivo e aggiornamento grafica  
        'If varETSDN.timerComunicationCan.ElapsedMilliseconds > 1000 Then
        '    If varETSDN.etsdnOnline Then
        '        ChangeControlVisibility(pBoxErrorCan, False)
        '        varETSDN.timerComunicationCan.Reset()
        '    Else
        '        ChangeControlVisibility(pBoxErrorCan, True)
        '    End If
        'End If
